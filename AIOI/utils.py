...
def CE_loss(y_true, y_pred):
    y_pred = tf.clip_by_value(y_pred, 1e-5, 1)
    y_truea = tf.clip_by_value(y_true, 1e-5, 1)
    return tf.losses.softmax_cross_entropy(y_truea, y_pred)


def adversary_loss(y_true, y_pred, eps):
    return CE_loss(y_true, y_pred) - eps


def eigan_loss(y_ally, y_advr, combined_output, eps=-np.log(0.5), alpha=1):
    ally_loss = CE_loss(y_ally, combined_output[0])
    advr_loss = adversary_loss(y_advr, combined_output[1], eps)
    c_loss = ally_loss - alpha * advr_loss
    return c_loss, ally_loss, advr_loss
